...
세 번째/네 번째 커뮤니티 피드백에서 추출. NoodlesOnTuesday (r/technicalanalysis) 제안.

> 원문 요약: "ATR 퍼센타일 랭킹으로 레짐 분류. ATR 기울기로 전환 감지.
> 완전 차단보다 포지션 축소가 더 많은 초기 진입을 잡는다.
> 백테스트를 ATR 퍼센타일 버킷별로 분리하면 레짐 의존성이 드러난다."

### 12. ATR Percentile Regime Classification — P1

**현재:** 변동성 레짐 구분 없음. 모든 시그널이 동일 조건에서 평가됨.
**개선:** 현재 ATR_14를 해당 심볼의 90일 ATR 히스토리 대비 퍼센타일로 변환.

**분류 기준:**
- 하위 25% → LOW_VOL (조용한 시장, 시그널 신뢰도 낮을 수 있음)
- 25-75% → NORMAL
- 상위 75% → HIGH_VOL (활발한 시장, 패턴이 더 선명)

**적용:** 파이프라인에서 레짐 태그를 시그널에 부착. HIGH_VOL 시그널에 보너스, LOW_VOL에 패널티.
**구현 파일:** `internal/indicator/` (ATR 히스토리 퍼센타일 계산) + `internal/pipeline/` (레짐 보너스)
**의존:** DB에서 과거 ATR 값 조회 필요 (또는 현재 봉 데이터에서 90일치 ATR 계산)
**상태:** 미착수

---

### 13. ATR Slope (Regime Transition Detector) — P2

**현재:** ATR 값의 방향성 정보 없음.
**개선:** ATR_14에 20기간 EMA를 씌워서 기울기 방향 감지.

**의미:**
- ATR 기울기 상승 → 변동성 확대 중 (무언가 시작되고 있음)
- ATR 기울기 하락 → 변동성 축소 중 (안정기 진입)
- 저변동 + 상승 기울기 → "coiled" 상태, 급격한 레짐 전환 임박

**적용:** 기울기 상승 중이면 시그널 민감도를 높여 초기 진입을 잡음.
**구현 파일:** `internal/indicator/indicator.go` (ATR EMA 추가)
**상태:** 미착수

---

### 14. HTF Filter 점수 감소 방식으로 전환 — P1

**현재:** 역추세 1H/4H 시그널을 완전 제거 (binary suppress).
**개선:** 완전 제거 대신 score를 50% 감소. "틀릴 수 있으니 건너뛰자" → "틀릴 수 있으니 작게 가자".

**근거:** NoodlesOnTuesday 경험 — 완전 차단이 너무 많은 초기 진입을 놓침.
특히 Wyckoff accumulation 초기에 EMA가 아직 bearish일 때 LONG 기회를 모두 놓치는 문제.

**구현:** `filterHTFContext()` → `penalizeHTFContext()` 로 변경. 시그널을 제거하지 않고 Score *= 0.5.
**구현 파일:** `internal/pipeline/pipeline.go`
**상태:** 미착수

---

### 15. 백테스트 ATR 퍼센타일 버킷 분리 — P2

**현재:** 백테스트가 전체 기간을 하나로 평가. 변동성 높은/낮은 기간이 평균화됨.
**개선:** ATR 퍼센타일로 봉을 버킷(LOW/NORMAL/HIGH)으로 나누고, 각 버킷별 성과를 분리 출력.

**가치:** "이 룰이 상위 25% 변동성에서만 작동한다"는 정보는 전체 백테스트가 평범해 보여도 유용.
**구현 파일:** `internal/backtest/` (버킷 분류 + 분리 통계)
**프론트엔드:** BacktestTab에 레짐별 탭 또는 테이블 추가
**상태:** 미착수

---

### 16. 실현 vs 내재 변동성 비교 — P3 (장기)

**현재:** 옵션 데이터 소스 없음.
**개선:** 실현 변동성(ATR 기반)과 내재 변동성(옵션 체인)을 비교. 실현 << 내재이면 "coiled" 상태.
**의존:** 옵션 데이터 소스 통합 필요 (CBOE, Yahoo Options, etc.)
**상태:** 미착수 — 데이터 소스 확보 후 진행
...
| 12  | ATR Percentile Regime         | P1       | M      | 높음 — 변동성 레짐별 시그널 품질 차별화 | 미착수 |
| 14  | HTF Filter 점수 감소 전환      | P1       | S      | 높음 — 초기 진입 놓침 방지         | 미착수 |
...
| 13  | ATR Slope 전환 감지            | P2       | S      | 중간 — 레짐 전환 조기 감지         | 미착수 |
| 15  | 백테스트 레짐 버킷 분리          | P2       | M      | 중간 — 룰별 레짐 의존성 분석        | 미착수 |
...
| 16  | 실현 vs 내재 변동성             | P3       | L      | 중간 — coiled 시장 감지 (장기)   | 미착수 |
