...
Owner 요청이 들어오면 Orchestrator가 먼저 회의를 열고, **관련 에이전트만** 순서대로 발언한다.
발언은 각 에이전트 역할 관점에서 **핵심만 2-3줄**로 제한한다.
...
📊 Quant  (수치/알고리즘 관점, 해당 시)
  [파라미터·데이터 관련 의견]

📈 TraderAdvisor  (실전 트레이더 관점, 해당 시)
  [실용성·신호 품질 관련 의견]

🎨 Designer  (UI/UX 관점, 해당 시)
  [화면 설계·팔레트 관련 의견]

🔧 Developer  (구현 관점, 항상 포함)
  [구현 접근법·예상 이슈]
...
- 회의 후 Orchestrator가 결정을 내리고 즉시 구현 진행
- 관련 없는 에이전트는 발언하지 않는다 (예: 백엔드 전용 작업에 Designer 불필요)
- 회의 내용은 별도 저장 없이 응답에만 표시 (문서화는 Recorder가 CHANGELOG에 반영)
...
*AGENTS.md v0.4 — 2026-03-14 — 팀 회의 프로토콜 추가 (섹션 9), 섹션 번호 갱신*
